/**
 * @file
 * @brief Sample implementation of an AllJoyn service that advertises, waits for 5 seconds and then stops
 * advertising.
 *
 *
 */
...
int main(int argc, char** argv, char** envArg)
{
    ...
    // Stop Advertising
    if (ER_OK == status) {
        status = g_msgBus->CancelAdvertiseName(SERVICE_NAME, opts.transports);
        if (status != ER_OK) {
